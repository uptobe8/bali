(() => {
  ...
  style.textContent = `
    .nusa-hero-video-card {
      width: 100%;
      max-width: 100%;
      border-radius: 28px;
      overflow: hidden;
    }
    .nusa-hero-video-wrap {
      position: relative;
      width: 100%;
      min-height: 460px;
      overflow: hidden;
    ...
    .nusa-hero-video-wrap video,
    .nusa-hero-video-wrap > img {
    ...
    .nusa-hero-video-wrap::before {
      content: "";
      position: absolute;
      inset: 0;
      z-index: 1;
      background: linear-gradient(90deg, rgba(2,16,11,.82), rgba(2,16,11,.42) 46%, rgba(2,16,11,.18)), linear-gradient(180deg, rgba(2,16,11,.25), rgba(2,16,11,.78));
      pointer-events: none;
    }
    .nusa-hero-video-wrap > .absolute.inset-0.z-\[1\] {
      z-index: 2 !important;
      justify-content: flex-end !important;
      padding: 34px !important;
      max-width: 760px;
    }
    .nusa-hero-video-wrap h1 {
    ...
    .nusa-sound-toggle.on {
      background: linear-gradient(135deg, rgba(0,200,117,.82), rgba(0,213,255,.78));
      color: #03100b;
    ...
    @media (max-width: 767px) {
      ...
      .nusa-hero-video-wrap::before {
        background: linear-gradient(180deg, rgba(2,16,11,.18), rgba(2,16,11,.32) 26%, rgba(2,16,11,.86));
      }
      .nusa-hero-video-wrap > .absolute.inset-0.z-\[1\] {
      ...
      .nusa-hero-video-wrap .grid.grid-cols-2 {
        gap: 10px !important;
  ...
  function tryPlay(audio, button) {
    audio.volume = 0.32;
    const p = audio.play();
    if (p && p.then) {
      p.then(() => {
        button.classList.add('on');
        button.textContent = '🔊 Selva ON';
      }).catch(() => {
        button.classList.remove('on');
        button.textContent = '🔇 Selva';
      });
    }
  }
