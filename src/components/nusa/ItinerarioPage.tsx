...
import { getItineraryOptions, activateItineraryOption, regenerateItineraryOption } from '@/lib/client/itinerary-options'
import { DayDetail } from './DayDetail'
import { EmptyTripState } from './EmptyTripState'
import { Skeleton } from '@/components/ui/skeleton'
import { Plane, Compass, WalletCards, CheckCircle2, RotateCcw, GitCompareArrows, Loader2 } from 'lucide-react'
...
export function ItinerarioPage({
  ...
  const { data, isLoading, isError } = useQuery({ queryKey: ['itinerary'], queryFn: getItinerary })
  const { data: optionsData } = useQuery({ queryKey: ['itinerary-options'], queryFn: getItineraryOptions, retry: false })
  ...
  const activeVariant = variants.find((v) => v.id === selectedVariantId) ?? variants.find((v) => v.id === 'media') ?? variants[0] ?? null
  ...
  const activateMutation = useMutation({
    ...
    onError: (err) => {
      toast.error('No se pudo cargar la opción', { description: err instanceof Error ? err.message : undefined })
    },
  })
  ...
  const regenerateMutation = useMutation({
    mutationFn: async (id: string) => {
      ...
      const res = await regenerateItineraryOption(id)
    ...
    onSuccess: async (res) => {
      ...
      await qc.invalidateQueries({ queryKey: ['itinerary'] })
      setSelectedId(null)
      toast.success('Opción regenerada', { description: res.message })
    },
    onError: (err) => {
      toast.error('No se pudo regenerar la opción', { description: err instanceof Error ? err.message : undefined })
    },
      ...
      <div className="nusa-paper min-h-full">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
          <header className="mb-8">
            ...
            <h1 className="font-display text-3xl sm:text-5xl text-ink mt-2 leading-tight">Tu viaje, contado día por día</h1>
  ...
  return (
    <div className="nusa-paper min-h-full">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        ...
        {isError && <div className="nusa-card-paper p-6 text-ink-soft">No pudimos cargar el itinerario. Reintenta en unos segundos.</div>}

        {regeneratingId && (
          <div className="mb-6 nusa-leaf-box flex items-center gap-3">
            <Loader2 className="w-5 h-5 animate-spin text-nusa-sun" />
            <div>
              <div className="kicker kicker-sun">Regenerando una opción</div>
              <p className="text-sm text-white/80 mt-1">La IA está rehaciendo solo esta versión. Puede tardar 2-3 minutos.</p>
            </div>
          </div>
        )}
          ...
          <section className="mb-8" id="comparar-opciones">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-ink-soft mt-1">Económica, media y premium respetan destino, duración, viajeros, ritmo, intereses y restricciones. Cambian coste, hoteles, comidas y logística.</p>
                </div>
              </div>
              {days.length > 0 && (
                <button onClick={() => document.getElementById('comparar-opciones')?.scrollIntoView({ behavior: 'smooth' })} className="nusa-btn-ghost inline-flex items-center gap-2 self-start sm:self-auto">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {options.map((o) => {
                ...
                const regeneratingThis = regeneratingId === o.id
                return (
                  <article key={o.id} className={`rounded-2xl border p-5 bg-white transition ${active ? 'border-ember shadow-[0_12px_34px_rgba(232,119,34,0.18)]' : 'border-leaf/12'}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="font-display text-2xl text-ink">{o.label}</div>
                        <span className={`mt-1 inline-flex rounded-full px-2.5 py-1 text-[11px] font-black uppercase tracking-wider ${o.recommended ? 'bg-ember text-white' : 'bg-leaf/10 text-leaf'}`}>{o.tag}{o.recommended ? ' · recomendada' : ''}</span>
                      </div>
                    ...
                    <div className="mt-4 grid gap-3 text-sm">
                      <div>
                        ...
                        <ul className="mt-1 list-disc pl-5 text-ink-soft space-y-1">{o.upgrades.map((x) => <li key={x}>{x}</li>)}</ul>
                      </div>
                      <div>
                        <div className="font-black text-ink">Sacrificios</div>
                        <ul className="mt-1 list-disc pl-5 text-ink-soft space-y-1">{o.tradeoffs.map((x) => <li key={x}>{x}</li>)}</ul>
                      </div>
                    </div>

                    <div className="mt-5 flex flex-col gap-2">
                      <button onClick={() => activateMutation.mutate(o.id)} disabled={activateMutation.isPending || regenerateMutation.isPending} className="nusa-btn-ember w-full disabled:opacity-60">
                        {active ? 'Volver a cargar esta opción' : 'Usar esta opción como itinerario'}
                      </button>
                      <button onClick={() => regenerateMutation.mutate(o.id)} disabled={activateMutation.isPending || regenerateMutation.isPending} className="nusa-btn-ghost w-full inline-flex items-center justify-center gap-2 disabled:opacity-60">
                        {regeneratingThis ? <Loader2 className="w-4 h-4 animate-spin" /> : <RotateCcw className="w-4 h-4" />}
                        {regeneratingThis ? 'Regenerando…' : 'Regenerar solo esta opción'}
                      </button>
                    </div>
                  </article>
                )
        ...
        {days.length > 0 && activeOption && (
          <section className="mb-6 nusa-leaf-box">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <span className="inline-flex self-start rounded-full bg-white/15 border border-white/20 px-3 py-1.5 text-xs font-black text-nusa-sun">{activeOption.perDayEstimate}</span>
            </div>
          </section>
        )}
...
function ItinerarioSkeleton() {
  return (
    <div className="grid lg:grid-cols-[300px_1fr] gap-6">
      <div className="space-y-2">{Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-16 rounded-xl bg-leaf/10" />)}</div>
      <div className="space-y-4">
        <Skeleton className="h-[360px] rounded-2xl bg-leaf/10" />
        <div className="grid sm:grid-cols-2 gap-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-24 rounded-xl bg-leaf/10" />)}</div>
      </div>
    </div>
  )
}
